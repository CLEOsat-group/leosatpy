...
class ReduceSatObs(object):
    ...
    def _check_closest_exposure(self, ic_all: ImageFileCollection,
        ...
        for i in range(len(exptime_check_imgtypes)):
            ...
            if ic_exptimes.summary is not None:
                ...
                for t in expt:
                    idx = np.argmin(np.abs(dark_exposures - t))
                    closest_dark_exposure = dark_exposures[idx]
                    has_nearest = True
                    if np.abs(t - closest_dark_exposure) > tolerance:
                        has_nearest = False
    ...
    def _run_ccdproc(self, obsparams: dict, filters: list, binnings: str):
        ...
        exptimes_list = self._check_closest_exposure(ic_all=ic_all,
                                                     obsparams=obsparams, tolerance=1)
            ...
            if self._config['DARK_CORRECT']:
                for i in range(2):
                    ...
                    for dexpt in dark_exptimes:
                        # print(dexpt, exptimes, ddark)
                        if dexpt not in ddark or dexpt not in exptimes:
                            continue
                        ...
                        if i == 0:
                            ccd_master_dark['sci_dark'][dark_exptime] = ccd_mdark, bias_corr, has_nearest
                        else:
                            ccd_master_dark['flat_dark'][dark_exptime] = ccd_mdark, bias_corr, has_nearest
    ...
    def _find_calibrations(self, telescope: str, obsparams: dict, date: datetime.date, filters: list,
                           binnings: tuple):
        ...
        dbias, ddark, dflat, dark_exptimes = self._create_master_fnames(ic_all,
                                                                        obsparams, filters,
                                                                        sci_bin_str)
        # print(dbias, ddark, dflat, dark_exptimes)
        ...
        create_dark = np.any(np.array([v[1] for _, v in ddark.items()]))
        if create_dark:
